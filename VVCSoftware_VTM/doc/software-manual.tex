...
for the VVC project. It applies to version 24.0 of the software.
\end{abstract}
...
ITU-T SG 21 and ISO/IEC SC29 WG5. One of the main goals of the
...
%\ShortOption{\None} &
\Default{1.0} &
Scaling ratio in horizontal direction for reference picture resampling. When GOPBasedRPR is true unless ratio is defined the ratio will be set to 2.0. When this scaling ratio or ScalingRatioVer is different from 1.0 and RPR is enabled a measure of PSNR1 is given in summary. PSNR1 is measured on the decoded picture resolution by calculating MSE for each picture, accumulating MSEs for all pictures and calculating average PSNR from the accumulated MSE.
\\

\Option{ScalingRatioVer} &
%\ShortOption{\None} &
\Default{1.0} &
Scaling ratio in vertical direction for reference picture resampling. When GOPBasedRPR is true unless ratio is defined the ratio will be set to 2.0. When this scaling ratio or ScalingRatioHor is different from 1.0 and RPR is enabled a measure of PSNR1 is given in summary. PSNR1 is measured on the decoded picture resolution by calculating MSE for each picture, accumulating MSEs for all pictures and calculating average PSNR from the accumulated MSE.
\\
...
\Option{RPRFunctionalityTesting} &
%\ShortOption{\None} &
\Default{false} &
Enables testing of RPR functionality according to defined order of resolutions from full resolution or one of three downscaled resolutions (default is $1/2$, $2/3$ and $4/5$ in both dimensions).
The order is defined in RPRSwitchingResolutionOrderList and QP settings in RPRSwitchingResolutionOrderList and number of frames for each resolution in
RPRSwitchingSegmentSize or according to RPRSwitchingTime if thats non-zero.
\\

\Option{RPRSwitchingResolutionOrderList} &
%\ShortOption{\None} &
\Default{"1, 0, 2, 0, 3, 0, 1, 0, 2, 0, 3, 0"} &
Order of resolutions for each segment for RPR functionality testing where 0,1,2,3 corresponds to full resolution,4/5,2/3 and 1/2.
\\

\Option{RPRSwitchingQPOffsetOrderList} &
%\ShortOption{\None} &
\Default{"-2, 0, -4, 0, -6, 0, -2, 0, -4, 0, -6, 0"} &
Order of QP offset for each segment for RPR functionality testing, where the QP is modified according to the given offset.
\\

\Option{RPRSwitchingSegmentSize} &
%\ShortOption{\None} &
\Default{32} &
Number of frames with same resolution for RPR functionality testing.
\\

\Option{RPRSwitchingTime} &
%\ShortOption{\None} &
\Default{0.0} &
Segment switching time in seconds for RPR functionality testing, when non-zero it defines the segment size according to frame rate (multiple of 8).
\\

\Option{RPRPopulatePPSatIntra} &
%\ShortOption{\None} &
\Default{false} &
Populate all PPS which can be used for RPR at the Intra, e.g. full-res, 4/5, 2/3 and 1/2.
\\
...
%\ShortOption{\None} &
\Default{0} &
Picture output options: output upscaled (2), decoded but in full resolution buffer (1) or decoded cropped (0, default) picture for reference picture resampling. When GOPBasedRPR is true it will be set to 2. When UpscaledOutput is equal to 2, PSNR for a picture after upscaling to full resolution is given first and PSNR in decoded resolution is given after size of decoded resolution. Otherwise PSNR in decoded resolution is given first and PSNR2 is given to represent PSNR after upscaling to full resolution.
\\

\Option{UpscaledOutputWidth} &
%\ShortOption{\None} &
\Default{0} &
When non-zero and UpscaledOutput is non-zero, overrides SPS and specifies upscaled output width
\\

\Option{UpscaledOutputHeight} &
%\ShortOption{\None} &
\Default{0} &
When non-zero and UpscaledOutput is non-zero, overrides SPS and specifies upscaled output height
\\
...
%\ShortOption{\None} &
\Default{false} &
Explicit specification of inter-layer reference pictures, using a layer-specific GOP structure (see section 3.1).
When ExplicitILRP is true, AllowablePredDirection should not be used or left to default values (all zero), and
inter-layer reference pictures are no longer inserted automatically.
\par
\\
...
A value of $-N$ implies a period that is a multiple $N$ based on frame rate $r$: $\mathrm{round}(r / N) \cdot N$. 
\\
...
\Option{LambdaScaleTowardsNextQP} &
\Default{0.0} &
Specifies a scaling of lambda that corresponds to a fractional QP offset from the slice QP as $2^(LambdaScaleTowardsNextQP/3)$.
Can be used for matching a target bitrate located inbetween the bitrate given by two adjacent base QPs.
Start from the base QP that gives a bitrate closest to target bitrate.
When the bitrate given by the base QP is above the target bitrate use a positive LambdaScaleTowardsNextQP to reduce the bitrate.
When the bitrate given by the base QP is below the target bitrate use a negative LambdaScaleTowardsNextQP to increase the bitrate.
A value of 0.5 gives approximately a bitrate given by the average of the bitrate for the base QP and the bitrate for the base QP + 1.
A value of -0.5 gives approximately a bitrate given by the average of the bitrate for the base QP and the bitrate for the base QP - 1.
\\
...
\Option{PPSInitialQPOffset} &
%\ShortOption{\None} &
\Default{0} &
Specifies a QP offset added when coding the initial QP value in the PPS.
This parameter may be set to a value different from 0 to minimize the amount of bits used for coding the initial QP value in the slice or picture header.
\\
...
%\ShortOption{\None} &
\Default{false} &
Enable CTU-level Lagrange multiplier and QP adaptation by estimating the Distortion Propagation Factor through pre-encoding.
\\

\Option{DPFKeyLength} &
%\ShortOption{\None} &
\Default{8} &
Distortion propagation length for calculating the Distortion Propagation Factor of key frames.
\\

\Option{DPFNonKeyLength} &
%\ShortOption{\None} &
\Default{0} &
Distortion propagation length for calculating the Distortion Propagation Factor of non-key frames.
\\
\end{OptionTableNoShorthand}
...
%\ShortOption{\None}&
\Default{false}&
Enables or disables encoder-side optimization with adaptive loop filter. When it is enabled, lagrange multiplier optimization is applied for chroma ALF and CCALF.
\\
...
%\ShortOption{\None} &
\Default{0} &
Enables explicit mutiple transform set (MTS).
0: disable,
...
%\ShortOption{\None} &
\Default{0} &
Enables implicit multiple transform set (MTS).
0: disable,
1: enable implicit intra MTS.
Must be 0 when MTS is nonzero. Setting MTS to 0 and MTSImplicit to 1 is equivalent to setting MTS to 4 and MTSImplicit to 0.
\\
...
%\ShortOption{\None} &
\Default{false} &
Enables or disables the use of affine inter mode.
0: disable,
1: enable affine inter mode
\\

\Option{AdaptBypassAffineMe} &
%\ShortOption{\None} &
\Default{false} &
Enables or disables the fast method which adaptively bypasses affine ME. 
\\

\Option{AffineAmvr} &
%\ShortOption{\None} &
\Default{false} &
Enables or disables the use of AMVR for affine inter mode.
\\

\Option{AffineAmvrEncOpt} &
%\ShortOption{\None} &
\Default{false} &
Enables or disables the encoder optimization of affine AMVR. 
\\

\Option{AffineAmvp} &
%\ShortOption{\None} &
\Default{true} &
Enables or disables the use of AMVP for affine inter mode when affine inter mode is used (enabled).
\\
...
\begin{SEIListTable}{List of Version 1 and RExt SEI messages}
   ...
   22 & Post-filter hint                         & Table \ref{tab:sei-post-filter hint} \\
  ...
  205 & Shutter Interval Information             & Table \ref{tab:sei-sii} \\
  207 & Constrained RASL encoding                & Table \ref{tab:sei-constrained-rasl-encoding} \\
  208 & Scalability Dimension Information        & Table \ref{tab:sei-sdi} \\
  ...
  226 & Packed Regions Info                      & Table \ref{tab:sei-pri}\\
  227 & Image Format Metadata                    & Table \ref{tab:sei-ifm}\\
\end{SEIListTable}
%%
%% SEI messages
%%
...
\Default{0} &
Control adaptive film grain parameter estimation - film grain analysis. If
enabled, log2ScaleFactor, intensity intervals and model parameters will be
determined by the encoder, based on a denoised input and a flat area mask,
either internally generated or externally provided (see SEIFGCExternalDenoised
and SEIFGCExternalMask)
\\
\Option{SEIFGCExternalMask} &
\Default{""} &
For film grain analysis, use this mask (yuv file) instead of internally
generated. Zero values represent flat areas. Must be the same bit depth and
chroma format as output.
\\
\Option{SEIFGCExternalDenoised} &
\Default{""} &
For film grain analysis, use this denoised video (yuv file) instead of
internally generated. Must be the same bit depth and chroma format as output.
\\
\Option{SEIFGCTemporalFilterPastRefs} &
\Default{"4"} &
When internally generating a denoised picture for film grain analysis, use this
number of past reference frames for the denoiser (specific to FGC analysis).
\\
\Option{SEIFGCTemporalFilterFutureRefs} &
\Default{"4"} &
When internally generating a denoised picture for film grain analysis, use this
number of future reference frames for the denoiser (specific to FGC analysis).
This should be set to zero in low-delay context.
\\
\Option{SEIFGCTemporalFilterStrengthFrame*} &
\Default{""} &
When internally generating a denoised picture for film grain analysis, use this
filtering strength every * frame for the denoiser (specific to FGC analysis),
where * is an integer. E.g. SEIFGCTemporalFilterStrengthFrame64 1.5 will
enable the denoiser at every 64th frame with strength 1.5. Longer intervals
overrides shorter when there are multiple matches.
If nothing is specified, the strength is set by default to 1.5 for
- every intra period in random-access mode
- every frame in all-intra
- every 2s in low-delay (i.e. intraPeriod < 1)
\\
...
\begin{tabular}{cp{0.35\textwidth}}
	...
	2 & Metadata for display adaptation through the use of attenuation maps\\
\end{tabular}
\\
...
\\
\Option{SEIGreenMetadataAMIFlags} &
\Default{0} &
Bitfield that specifies which parameters are needed to apply the attenuation map (if SEIGreenMetadataType == 2).
\par
\begin{tabular}{cp{0.35\textwidth}}
	0 & Indicates that attenuation map information follows  \\
	1 & Indicates that attenuation map information is defined globally for all the listed attenuation maps \\
	2 & Indicates that the listed attenuation maps can be used to approximate other attenuation maps for other reduction rates \\
	3 & Indicates that some preprocessing is required to use the listed attenuation maps \\
	4 & Indicates that backlight scaling is required \\
	5 & Indicates that video quality information is present \\
\end{tabular}
\\
\Option{SEIGreenMetadataAMIDisplayModel} &
\Default{0} &
Bitfield that specifies the display models for which the application of attenuation maps may be used (if SEIGreenMetadataType == 2).
\par
\begin{tabular}{cp{0.35\textwidth}}
	0 & Attenuation maps may be used for backlit displays  \\
	1 & Attenuation maps may be used for emissive displays  \\
\end{tabular}
\\
\Option{SEIGreenMetadataAMIApproximationModel} &
\Default{0} &
Specifies the model used to extrapolate the attenuation map samples for another energy reduction rate (if SEIGreenMetadataType == 2).
\par
\begin{tabular}{cp{0.35\textwidth}}
	0 & Linear scaling should be used to generate additional attenuation maps  \\
	1 & Bilinear interpolation should be used to generate additional attenuation maps \\
	2 & Lanczos interpolation should be used to generate additional attenuation maps  \\
	3 & Bicubic interpolation should be used to generate additional attenuation maps \\
	4 & User defined process \\
\end{tabular}
\\
\Option{SEIGreenMetadataAMIMapNumber} &
\Default{0} &
Specifies the number of auxiliary pictures of type AUX_ALPHA in the CVS. It corresponds to the number of attenuation maps (if SEIGreenMetadataType == 2).
\\
\Option{SEIGreenMetadataAMILayerId} &
\Default{0} &
Specifies the identifier of the decoded layer for the attenuation map of index i (if SEIGreenMetadataType == 2).
\\
\Option{SEIGreenMetadataAMIOlsNumber} &
\Default{0} &
Specifies the number of output layer sets to which the attenuation map of index i belongs (if SEIGreenMetadataType == 2).
\\
\Option{SEIGreenMetadataAMIOlsId} &
\Default{0} &
Specifies the identifier of the output layer set of index j for the attenuation map of index i (if SEIGreenMetadataType == 2).
\\
\Option{SEIGreenMetadataAMIEnergyReductionRate} &
\Default{0} &
Specifies the expected energy savings rate when the video is displayed after applying the attenuation map of index i (if SEIGreenMetadataType == 2).
\\
\Option{SEIGreenMetadataAMIVideoQualityMetricType} &
\Default{0} &
Specifies the quality metric which was considered to inform of the reduction of the video quality due to the application of the attenuation map of index i (if SEIGreenMetadataType == 2).
\par
\begin{tabular}{cp{0.35\textwidth}}
	0 & PSNR  \\
	1 & SSIM \\
...
\end{tabular}
\\
\Option{SEIGreenMetadataAMIVideoQualityLevel} &
\Default{0} &
Indicates the expected video quality when the video is rendered on a display after application of the attenuation map of index i (if SEIGreenMetadataType == 2).
\\
\Option{SEIGreenMetadataAMIMaxValue} &
\Default{0} &
Indicates the maximum value of the attenuation map of index i (if SEIGreenMetadataType == 2).
\\
\Option{SEIGreenMetadataAMIAttenuationUseIdc} &
\Default{0} &
Specifies the use of the attenuation map sample values of the decoded auxiliary picture of index i (if SEIGreenMetadataType == 2).
\par
\begin{tabular}{cp{0.35\textwidth}}
	0 & The attenuation map sample values should be substracted from the associated primary picture sample values  \\
	1 & The attenuation map sample values should be multiplied to the associated primary picture sample values \\
	2 & User defined process  \\
\end{tabular}
\\
\Option{SEIGreenMetadataAMIAttenuationCompIdc} &
\Default{0} &
Specifies on which colour component(s) of the associated primary picture the attenuation map of index i should be applied (if SEIGreenMetadataType == 2).
\par
\begin{tabular}{cp{0.35\textwidth}}
	0 & The luma component of the attenuation map is applied to the luma component of the associated primary picture  \\
	1 & The luma component of the attenuation map is applied to the luma and the chroma components of the associated primary picture  \\
	2 & The luma component of the attenuation map is applied to the three RGB components of the associated primary picture  \\
	3 & The luma component of the attenuation map is applied to the first component of the associated primary picture  \\
	4 & The luma component of the attenuation map is applied to the second component of the associated primary picture  \\
	5 & The luma component of the attenuation map is applied to the third component of the associated primary picture  \\
	6 & User defined process  \\
\end{tabular}
\\
\Option{SEIGreenMetadataAMIPreprocessingFlag} &
\Default{0} &
Specifies whether some pre-upsampling is to be used on the attenuation map of index i (if SEIGreenMetadataType == 2).
\\
\Option{SEIGreenMetadataAMIPreprocessingType} &
\Default{0} &
Specifies the recommended type of the interpolation used to resample the attenuation map on index i (if SEIGreenMetadataType == 2).
\par
\begin{tabular}{cp{0.35\textwidth}}
	0 & Bicubic interpolation should be used to resample the attenuation map sample values  \\
	1 & Bilinear interpolation should be used to resample the attenuation map sample values   \\
	2 & Lanczos interpolation should be used to resample the attenuation map sample values  \\
	3 & User defined process  \\
\end{tabular}
\\
\Option{SEIGreenMetadataAMIPreprocessingScaleIdc} &
\Default{0} &
Specifies which scaling should be applied to the attenuation map of index i before applying it on the decoded picture (if SEIGreenMetadataType == 2).
\par
\begin{tabular}{cp{0.35\textwidth}}
	0 & Scaling of $\frac{1}{255}$  \\
	1 & User defined process  \\
\end{tabular}
\\
\Option{SEIGreenMetadataAMIBacklightScalingIdc} &
\Default{0} &
Specifies the process to compute the scaling factor of the backlight of transmissive pixel displays, derived from the attenuation map of index i (if SEIGreenMetadataType == 2).
\par
\begin{tabular}{cp{0.35\textwidth}}
	0 & Scaling by the ratio between maximum values before and after applying the attenuation map  \\
	1 & User defined process  \\
\end{tabular}
\\
\end{OptionTableNoShorthand}
...
\begin{OptionTableNoShorthand}{Neural network post-filter characteristics}{tab:sei-nn-post-filter-characteristics}
  ...
  \\
  \Option{SEINNPFCNumFilters} &
  \Default{0} &
  Specifies the number of neural network post-filters.
  \\
  \Option{SEINNPFCId\emph{i}} &
  \Default{0} &
  Specifies the id of the \emph{i}-th neural network post-filter.
  \\
  \Option{SEINNPFCModeIdc\emph{i}} &
  \Default{0} &
  Specifies the nnpfc_mode_idc of the \emph{i}-th neural network post-filter.
  \\
  \Option{SEINNPFCUriTag{i}} &
  \Default{""} &
  specifies that the post-processing filter of the \emph{i}-th neural network post-filter is a neural network identified by a specified tag URI.
  \\
  \Option{SEINNPFCUri{i}} &
  \Default{""} &
  specifies that the post-processing filter of the \emph{i}-th neural network post-filter is a neural network information URI.
  \\
  \Option{SEINNPFCPropertyPresentFlag\emph{i}} &
  \Default{false} &
  ...
  \Default{false} &
  Specifies the values of outSubWidthC and outSubHeightC
  \par
    \begin{tabular}{cp{0.4\textwidth}}
      true & outSubWidthC is equal to 1 and outSubHeightC is equal to 1 \\
      false & outSubWidthC is equal to 2 and outSubHeightC is equal to 1 \\
    \end{tabular}
    \\
  \\
  \Option{SEINNPFCOutColourFormatIdc\emph{i}} &
  \Default{0} &
  ...
  \end{tabular}
  \\
  \Option{SEINNPFCPicWidthNumerator\emph{i}} &
  \Default{1} &
  Specifies the output picture width numerator (relative to the input picture size) for the \emph{i}-th neural network post-filter.
  \\
  \Option{SEINNPFCPicWidthDenominator\emph{i}} &
  \Default{1} &
  Specifies the output picture width denominator (relative to the input picture size) for the \emph{i}-th neural network post-filter.
  \\
  \Option{SEINNPFCPicHeightNumerator\emph{i}} &
  \Default{1} &
  Specifies the output picture height numerator (relative to the input picture size) for the \emph{i}-th neural network post-filter.
  \\
  \Option{SEINNPFCPicHeightDenominator\emph{i}} &
  \Default{1} &
  Specifies the output picture height denominator (relative to the input picture size) for the \emph{i}-th neural network post-filter.
  \\
  \Option{SEINNPFCComponentLastFlag\emph{i}} &
  \Default{false} &
  ...
  \end{tabular}
  \\
  \Option{SEINNPFCInpFormatIdc\emph{i}} &
  \Default{0} &
  Specifies the method of converting a sample value of the decoded picture to an input value to the \emph{i}-th neural network post-filter.
  \par
    \begin{tabular}{cp{0.45\textwidth}}
      0 & Real numbers where the value range is 0 to 1, inclusive. \\
      1 & Unsigned integer value range of 0 to the bit depth indicated for the input tensor (see syntax element below).
    \end{tabular}
  \\
  \Option{SEINNPFCInpTensorBitLumaDepthMinusEight\emph{i}} &
  \Default{0} &
  Specifies the bit depth of the input luma tensor - 8 for the \emph{i}-th neural network post-filter, when nnpfc_inp_format_idc is equal to 1.
  \\
  \Option{SEINNPFCInpTensorBitDepthChromaMinusEight\emph{i}} &
  \Default{0} &
  Specifies the bit depth of the input chroma tensor - 8 for the \emph{i}-th neural network post-filter, when nnpfc_inp_format_idc is equal to 1.
  \\
  \Option{SEINNPFCAuxInpIdc\emph{i}} &
  \Default{0} &
  ...
  \\
  \Option{SEINNPFCFullRangeFlag\emph{i}} & 
  \Default{false} &
  Specifies scaling and offset values applied in association with the matrix coefficients as specified by nnpfc_matrix_coeff.
  \par
    Semantics of nnpfc_full_range_flag are as specified for the VideoFullRangeFlag parameter in Rec. ITU-T H.273 | ISO/IEC 23091-2
  \\
  \Option{SEINNPFCColPrimaries\emph{i}} &
  \Default{0} &
  ...
  \\
  \Option{SEINNPFCTransCharacteristics\emph{i}} &
  \Default{0} &
  ...
  \\
  \Option{SEINNPFCMatrixCoeffs\emph{i}} &
  \Default{0} &
  ...
  \\
  \Option{SEINNPFCInpOrderIdc\emph{i}} &
  \Default{0} &
  ...
  \end{tabular}
  \\
  \Option{SEINNPFCOutFormatIdc\emph{i}} &
  \Default{0} &
  Specifies the sample values output by the \emph{i}-th neural network post-filter.
  \par
  \begin{tabular}{cp{0.46\textwidth}}
    0 & Real numbers where the value range is 0 to 1, inclusive. \\
    1 & Unsigned integer numbers where the value range is 0 to $(1 << bitDepth) - 1$ inclusive.
  \end{tabular}
  \\
  \Option{SEINNPFCOutTensorBitDepthLumaMinusEight\emph{i}} &
  \Default{0} &
  Specifies the bit depth of the output luma tensor - 8 for the \emph{i}-th neural network post-filter, when nnpfc_out_format_idc is equal to 1.
  \\
  \Option{SEINNPFCOutTensorBitDepthChromaMinusEight\emph{i}} &
  \Default{0} &
  Specifies the bit depth of the output chroma tensor - 8 for the \emph{i}-th neural network post-filter, when nnpfc_out_format_idc is equal to 1.
  \\
  \Option{SEINNPFCOutOrderIdc\emph{i}} &
  \Default{0} &
  ...
  \end{tabular}
  \\
  \Option{SEINNPFCChromaLocInfoPresentFlag\emph{i}} &
  \Default{false} &
  Specifies nnpfc_chroma_loc_info_present_flag of the \emph{i}-th neural network post-filter.
  \par
    When true (non-zero) specifies the presence of the nnpfc_chroma_sample_loc_type_frame syntax element in the NNPFC SEI message.
  \par
    When false specifies the absence of the nnpfc_chroma_sample_loc_type_frame syntax element in the NNPFC SEI message.
  \\
  \Option{SEINNPFCChromaSampleLocTypeFrame\emph{i}} &
  \Default{0} &
  Specifies the location of chroma samples of the output pictures for the \emph{i}-th neural network post-filter.
  \par
  \begin{tabular}{cp{0.46\textwidth}}
    0 & Left \\
    1 & Center \\
  ...
  \end{tabular}
  \\
  \Option{SEINNPFCConstantPatchSizeFlag\emph{i}} &
  \Default{false} &
  ...
  \\
  \Option{SEINNPFCPatchWidthMinus1\emph{i}} &
  \Default{0} &
  ...
  \\
  \Option{SEINNPFCPatchHeightMinus1\emph{i}} &
  \Default{0} &
  ...
  \\
  \Option{SEINNPFCExtendedPatchWidthCdDeltaMinus1\emph{i}} &
  \Default{0} &
  ...
  \\
  \Option{SEINNPFCExtendedPatchHeightCdDeltaMinus1\emph{i}} &
  \Default{0} &
  ...
  \\
  \Option{SEINNPFCPaddingType\emph{i}} &
  \Default{0} &
  ...
  \end{tabular}
  \\
  \Option{SEINNPFCLog2ParameterBitLengthMinus3\emph{i}} &
  \Default{0} &
  ...
  \\
  \Option{SEINNPFCScanTypeIdc\emph{i}} &
  \Default{0} &
  Specifies the preferred display method for the pictures output by the NNPF: not overscan (2), overscan (1), unknown or unspecified (0).
  \\
  \Option{SEINNPFCForHumanViewingIdc\emph{i}} &
  \Default{0} &
  Specifies the user viewing usage level for the \emph{i}-th neural network post-filter: optimal for human viewing (3), suitable (2), unsuitable (1), unknown (0).
  \\
  \Option{SEINNPFCForMachineAnalysisIdc\emph{i}} &
  \Default{0} &
  Specifies the machine analysis usage level for the \emph{i}-th neural network post-filter: optimal for machine analysis (3), suitable (2), unsuitable (1), unknown (0).
  \\
  \Option{SEINNPFCNumParametersIdc\emph{i}} &
  \Default{0} &
  ...
  \\
  \Option{SEINNPFCNumKmacOperationsIdc\emph{i}} &
  \Default{0} &
  ...
  \\
  \Option{SEINNPFCTotalKilobyteSize\emph{i}} &
  \Default{0} &
  ...
  \\
  \Option{SEINNPFCPayloadFilename\emph{i}} &
  \Default{""} &
  Specifies the NNR bitstream of the \emph{i}-th neural network post-filter.
  \\
  \Option{SEINNPFCNumberInputDecodedPicsMinusOne\emph{i}} &
  \Default{0} &
  Specifies the number of decoded output pictures minus 1 used as input for the \emph{i}-th neural network post-filter. 
  \\
  \Option{SEINNPFCNumberInterpolatedPics\emph{i}} &
  \Default{0} &
  Specifies a list, where the j-th entry in the list specifies interpolated pictures generated by the \emph{i}-th neural network post-filter between the j-th and (j+1)-th picture used as input for the post processing filter.
  \\
  \Option{SEINNPFCNumberExtrapolatedPicsMinus1\emph{i}} &
  \Default{0} &
  ...
  \\
  \Option{SEINNPFCAbsentInputPicZeroFlag\emph{i}} &
  \Default{false} &
  Specifies the nnpfc_absent_input_pic_zero_flag of the \emph{i}-th neural network post-filter.
  \\
  \Option{SEINNPFCInbandSeedFlag\emph{i}} &
  \Default{false} &
  Specifies the nnpfc_inband_seed_flag of the \emph{i}-th neural network post-filter.
  \\
  \Option{SEINNPFCSeed\emph{i}} &
  \Default{0} &
  Indicates the seed value used as input for the \emph{i}-th neural network post-filter.
  \\
\end{OptionTableNoShorthand}

\begin{OptionTableNoShorthand}{Neural network post-filter activation}{tab:sei-nn-post-filter-activation}
  ...
  \\
  \Option{SEINNPostFilterActivationTargetId} &
  \Default{0} &
  Specifies the id of the neural network post-filter.
  \\
  \Option{SEINNPostFilterActivationCancelFlag} &
  \Default{false} &
  Indicates that the NNPFA SEI message cancels the persistence (true) or follows (false).
  \\
  \Option{SEINNPostFilterActivationTargetBaseFlag} &
  \Default{false} &
  Specifies that the target NNPF is the base NNPF.
  \\
  \Option{SEINNPostFilterActivationPersistenceFlag} &
  \Default{false} &
  Specifies the persistence of the target neural-network post-processing filter for the current layer.
  \\
  \Option{SEINNPostFilterActivationNoPrevCLVSFlag} &
  \Default{false} &
  Specifies whether input pictures cannot (true) or can (false) originate from a previous CLVS.
  \\
  \Option{SEINNPostFilterActivationNoFollCLVSFlag} &
  \Default{false} &
  Specifies whether input pictures cannot (true) or can (false) originate from a following CLVS.
  \\
  \Option{SEINNPostFilterActivationOutputFlag} &
  \Default{\NotSet} &
  ...
  \\
  \Option{SEINNPostFilterActivationSelectedInputFlag} &
  \Default{0} &
  Specifies whether input pic shift is present or not.
  \\
  \Option{SEINNPostFilterActivationSeedUpdateFlag} &
  \Default{false} &
  Specifies whether seed update is present or not.
  \\
  \Option{SEINNPostFilterActivationSeed} &
  \Default{0} &
  Specifies updated seed, if present.
  \\
...
\Option{SEIPOId} &
\Default{0} &
Specifies the id of the SEI processing order SEI message.
\\
\Option{SEIPOForHumanViewingIdc} &
\Default{0} &
Specifies the user viewing usage level of video resulting from processing chain specified by SPO SEI: optimal for human viewing (3), suitable (2), unsuitable (1), unknown (0, default).
\\
\Option{SEIPOForMachineAnalysisIdc} &
\Default{0} &
Specifies the machine analysis usage level of video resulting from processing chain specified by SPO SEI: optimal for machine analysis (3), suitable (2), unsuitable (1), unknown (0, default).
\\
\Option{SEIPONumMinus1} &
\Default{0} &
Specifies the number of SEIs minus 1 in SEI processing order SEI message. 
\\
\Option{SEIPOBreadthFirstFlag} &
\Default{0} &
Specifies that breadth-first handling of processing chain is applied (1), or that either breadth-first or depth-first can be applied (0, default).
\\
\Option{SEIPOWrappingFlag\emph{i}} &
\Default{false} &
...
\Default{0} &
Specifies the SEIPONumofPrefixByte is present for the \emph{i}-th SEI message for which information is provided in the SEI processing order SEI message.
\\
...
\\
\Option{SEIPONumofPrefixBits\emph{i}} &
\Default{0} &
Specifies the number of prefix bits for the \emph{i}-th SEI message present in processing order SEI message.
\\
\Option{SEIPOPrefixByte\emph{i}} &
\Default{0} &
Specifies the \emph{i}-th prefix byte present in processing order SEI message.
\\
\Option{SEIPOComplexityInfoPresentFlag} &
\Default{false} &
Specifies whether complexity info is present (1) or not (0). Default value: 0.
\\
\Option{SEIPOParameterTypeIdc} &
\Default{0} &
Specifies type of parameters of the NNPFs (0) only integer, (1) integer or floating point, (2) binary only, (3) reserved. Default value: 0.
\\
\Option{SEIPOLog2ParameterBitLengthMinus3} &
\Default{0} &
0, 1, 2, and 3 means the NNPFs do not use parameters of bit length greater than 8, 16, 32, and 64, respectively. Default value: 0.
\\
\Option{SEIPONumParametersIdc} &
\Default{0} &
Specifies max number of parameters needed by NNPFs in the processing chain. Default value: 0.
\\
\Option{SEIPONumKmacOperationIdcg} &
\Default{0} &
When greater than 0 specifies that the max number of multiply-accumulate operations per sample of the NNPFs is less than or equal to po_num_kmac_operations_idc * 1000. ) means unknown. Default value: 0.
\\
\Option{SEIPOTotalKilobyteSize} &
\Default{0} &
When greater than 0 specifies a total size in kilobytes required to store the uncompressed parameters for NNPFs. 0 means unknown. Default value: 0.
\\
\end{OptionTableNoShorthand}

\begin{OptionTableNoShorthand}{Encoder optimization info SEI message encoder parameters}{tab:sei-encoder-optimization-info}
\Option{SEIEOIEnabled} &
\Default{1} &
Specifies whether encoder optimization info SEI message to be generated or not.
\\
\Option{SEIEOICancelFlag} &
\Default{0} &
Specifies whether this SEI message cancels the previous encoder optimization info SEI message.
\\
\Option{SEIEOIPersistenceFlag} &
\Default{1} &
Specifies whether this SEI message applies to just one picture or sequence of pictures.
\\
\Option{SEIEOIForHumanViewingIdc} &
\Default{1} &
Specifies whether the optimized result is suitable for human viewing.
\\
\Option{SEIEOIForMachineAnalysiIdc} &
\Default{1} &
...
\\
\Option{SEIEOINumIntPics} &
\Default{2} &
Specifies the number of added picture between two pics when temporal resampling type optimization is enabled.
\\
\Option{SEIEOISrcPicFlag} &
\Default{1} &
Value of 1 specifies that the picture in the same access unit that contains the EOI SEI message is a source picture. Value of 0 provides no such indication.
\\
\Option{SEIEOIOrigPicDimensionsFlag} &
\Default{0} &
Specifies if original source picture dimensions are present.
\\
\Option{SEIEOIOrigPicWidthMinus1} &
\Default{0} &
Specifies the width of the original source picture minus 1.
\\
\Option{SEIEOIOrigPicHeightMinus1} &
\Default{0} &
Specifies the height of the original source picture minus 1.
\\
\Option{SEIEOISpatialHorResamplingTypeIdc} &
\Default{0} &
Specifies spatial horizontal resampling type, 0: no spatial resampling; 1: spatial subsampling; 2: spatial upsampling.
\\
\Option{SEIEOISpatialVerResamplingTypeIdc} &
\Default{0} &
Specifies spatial vertical resampling type, 0: no spatial resampling; 1: spatial subsampling; 2: spatial upsampling.
\\
...
\begin{OptionTableNoShorthand}{Text description information}{tab:sei-text-description-information}
\Option{SEITextDescriptionID} &
\Default{1} &
...
\Option{SEIDSCId} &
\Default{0} &
Identifying number for Digitally Signed Content Initialisation, Selection and Verification SEI message.
\\
...
\\
\Option{SEIDSCSignAURSEI} &
\Default{false} &
Enable signing of AUR SEI for Digitally Signed Content SEI messages.
\\
\Option{SEIDSCSignGFVSEI} &
\Default{false} &
Enable signing of GFV SEI for Digitally Signed Content SEI messages.
\\
\Option{SEIDSCSignGFVESEI} &
\Default{false} &
Enable signing of GFVE SEI for Digitally Signed Content SEI messages.
\\
\Option{SEIDSCSignNNPFCSEI} &
\Default{false} &
Enable signing of NNPFC SEI for Digitally Signed Content SEI messages.
\\
\Option{SEIDSCSignNNPFASEI} &
\Default{false} &
Enable signing of NNPFA SEI for Digitally Signed Content SEI messages.
\\
\end{OptionTableNoShorthand}
...
\\
\Option{SEIOmiTolerancePresentFlag} &
\Default{false} &
If true, indicates that omi_aux_sample_tolerance syntax element is present.
\\
\Option{SEIOmiAuxSampleTolerance[i]} &
\Default{0} &
Specifies a tolerance range for mapping of decoded sample values of the i-th auxiliary picture to object masks.
\\
\end{OptionTableNoShorthand}


\begin{OptionTableNoShorthand}{Generative face video SEI message encoder parameters}
{tab:sei-gfv}
  \Option{--} &
  \Default{--} &
  \textbf{The configuration files for generative face video SEI message can be generated by the AhG16 GFVC common software in https://vcgit.hhi.fraunhofer.de/jvet-ahg-gfvc/gfvc_v1/-/blob/AhG16_GFVC_Software_v3/source/GFVC/codecs_utils.py. }
  \\
  \Option{SEIGenerativeFaceVideoEnabled} &
  \Default{false} &
  Enables (true) or disables (false) the insertion of generative face video (GFV) information SEI message.
  \\
  \Option{SEIGenerativeFaceVideoNumber} &
  \Default{0} &
  Controls how many generative face video (GFV) information SEI message to be carried by one base picture.
  \\
  \Option{SEIGenerativeFaceVideoBasePicFlag} &
  \Default{false} &
  When true, indicates the current decoded output picture corresponds to a base picture. When false, indicates the current decoded output picture does not correspond to a base picture. 
  \\
  \Option{SEIGenerativeFaceVideoNNPresentFlag} &
  \Default{false} &
  When true, indicates a neural network that may be used as a TranslatorNN( ) is contained or indicated by the SEI message. When false, indicates a neural network that may be used as a TranslatorNN( ) is not contained or indicated by the SEI message.
  \\
  \Option{SEIGenerativeFaceVideoNNModeIdc} &
  \Default{0} &
  Specifies a neural network that may be used as a TranslatorNN() and has the same syntax and semantics with nnpfc_mode_idc of neural network post-filter.
  \\
  \Option{SEIGenerativeFaceVideoNNTagURI} &
  \Default{``''} &
  Specifies a neural network that may be used as a TranslatorNN() and has the same syntax and semantics with nnpfc_tag_uri of neural network post-filter identified by a specified tag URI.
  \\
  \Option{SEIGenerativeFaceVideoNNURI} &
  \Default{``''} &
  Specifies a neural network that may be used as a TranslatorNN() and has the same syntax and semantics with nnpfc_uri of neural network post-filter information URI.
  \\
  \Option{SEIGenerativeFaceVideoId} &
  \Default{``0''} &
  ...
  \\
  \Option{SEIGenerativeFaceVideoCnt} &
  \Default{``''} &
  Specifies a GFV SEI message instance count value for this gfv_id value within a picture unit. List of count numbers with the corresponding number of specified SEIGenerativeFaceVideoNumber.
  \\ \Option{SEIGenerativeFaceVideoChromaKeyInfoPresentFlag} &
  \Default{0} &
   When present, equal to 1 indicates that the syntax elements gfv_chroma_key_value_present_flag[c] and gfv_chroma_key_thr_present_flag[i] are present and the syntax elements and gfv_chroma_key_value[c] and gfv_chroma_key_thr_value[i] might be present.
  \\
  \Option{SEIGenerativeFaceVideoChromaKeyPurpose idc} &
  \Default{0} &
   Specifies the chroma key purpose: unspecified (0), background (1), face recongnition (2).
  \\  \Option{SEIGenerativeFaceVideoChromaKeyValuePresentFlag} &
  \Default{``''} &
   When present, equal to 1 indicates that the syntax element gfv_chroma_key_value[c] is present.
  \\
  \Option{SEIGenerativeFaceVideoChromaKeyValue} &
  \Default{``''} &
   Specifies the chroma key value corresponding to the c-th colour component.   
  \\
  \Option{SEIGenerativeFaceVideoChromaKeyThrPresentFlag} &
  \Default{``''} &
   When present, equal to 1 indicates that the syntax element gfv_chroma_thr_lower and gfv_chroma_thr_upper_delta_minus1 are present.
  \\
  \Option{SEIGenerativeFaceVideoChromaKeyThrLower} &
  \Default{``''} &
   When present, specifies the chroma key lower threshold value.  
  \\
  \Option{SEIGenerativeFaceVideoChromaKeyThrUpperDeltaMinus1} &
  \Default{``''} &
   When present, specifies the chroma key upper threshold value.
  \\
  \Option{SEIGenerativeFaceVideoFusionPicFlag} &
  \Default{``''} &
  When present, equal to 1 indicates the current decoded picture, which corresponds to a driving picture that may be used for fusion, may be input to GenerativeNN( ). gfv_fusion_pic_flag equal to 0 indicates the current decoded picture should not be input to GenerativeNN( ). List of fusion flags  with the corresponding number of specified SEIGenerativeFaceVideoNumber. 
  \\
  \Option{SEIGenerativeFaceVideoLowConfidenceFaceParameterFlag} &
  \Default{``''} &
  When present, indicates the facial parameters have been derived with low confidence. When false, indicates the confidence information of the facial parameters is not specified. List of low confidence faceParameter flags with the corresponding number of specified SEIGenerativeFaceVideoNumber.     
  \\
  \Option{SEIGenerativeFaceVideoCoordinatePresentFlag} &
  \Default{``''} &
    When true, indicates that coordinate information of keypoints is present. When false, indicates that coordinate information of keypoints is not present. List of coordinate present flags with the corresponding number of specified SEIGenerativeFaceVideoNumber.
  \\
  \Option{SEIGenerativeFaceVideoCoordinateQuantizationFactor} &
  \Default{``''} &
  Specifies the precision factor (the length \& in bits,) to process the facial coordinate parameters. The value range may not exceed 32. List of coordinate quantization factors with the corresponding number of specified SEIGenerativeFaceVideoNumber.
  \\ 
  \Option{SEIGenerativeFaceVideoCoordinatePredFlag} &
  \Default{``''} &
  Determines whether the coordinate parameter is inter-predicted. List of coordinate prediction factors with the corresponding number of specified SEIGenerativeFaceVideoNumber.
  \\ 
  \Option{SEIGenerativeFaceVideo3DCoordinateFlag} &
  \Default{``''} &
  When true, specifies 3D coordinate-type paramters; otherwise 2D coordinate-type parameters. List of 3D coordinate factors with the corresponding number of specified SEIGenerativeFaceVideoNumber.
  \\  
  \Option{SEIGenerativeFaceVideoCoordinatePointNum} &
  \Default{``''} &
  Specifies the number of facial coordinate parameters. List of coordinate  numbers with the corresponding number of specified SEIGenerativeFaceVideoNumber.
  \\  
  \Option{SEIGenerativeFaceVideoXCoordinate} &
  \Default{``''} &
  List of the x-axis coordinate with the floating type.
  \\   
  \Option{SEIGenerativeFaceVideoYCoordinate} &
  \Default{``''} &
  List of the y-axis coordinate with the floating type.
  \\   
  \Option{SEIGenerativeFaceVideoZCoordinateMaxValue} &
  \Default{``''} &
  Indicates the maximum absolute value of z-axis coordinates of keypoints.  List of z-axis coordinate maximum values  with the corresponding number of specified SEIGenerativeFaceVideoNumber.
  \\    
  \Option{SEIGenerativeFaceVideoZCoordinate} &
  \Default{``''} &
  List of the z-axis coordinate with the floating type.
  \\   
  \Option{SEIGenerativeFaceVideoMatrixPresentFlag} &
  \Default{``''} &
  When true, indicates that information of  matrix parameters is present. When false, indicates that cinformation of  matrix parameters is not present. List of matrix present flags with the corresponding number of specified SEIGenerativeFaceVideoNumber.
  \\
  \Option{SEIGenerativeFaceVideoMatrixElementPrecisionFactor} &
  \Default{``''} &
  Specifies the precision factor (the length \& in bits,) to process the facial matrix parameters. The value range may not exceed 32. List of matrix quantization factors with the corresponding number of specified SEIGenerativeFaceVideoNumber.
  \\ 
  \Option{SEIGenerativeFaceVideoMatrixPredFlag} &
  \Default{``''} &
  Determines whether the matrix parameter is inter-predicted. List of matrix prediction factors with the corresponding number of specified SEIGenerativeFaceVideoNumber.
  \\
  \Option{SEIGenerativeFaceVideoNumMatrixType} &
  \Default{``''} &
  Indicates the number of matrix types signalled in the SEI message. List of NumMatrixType with the corresponding number of specified SEIGenerativeFaceVideoNumber.
  \\
   \Option{SEIGenerativeFaceVideoMatrixTypeIdx} &
  \Default{``''} &
  Indicates the index of the i-th matrix type. List of MatrixTypeIdx with the corresponding number of specified SEIGenerativeFaceVideoNumMatrixType.
  \par
  \begin{tabular}{cp{0.43\textwidth}}
    0 & Affine translation matrix with the size of 2$\times$2 or 3$\times$3.  \\
    1 & Covariance matrix with size of 2$\times$2 or 3$\times$3. \\
    2 & Mouth matrix representing mouth motion. \\
    3 & Eye matrix representing the open-close status and level of eyes. \\ 
    4 & Head rotation parameters with the size of 2$\times$2 or 3$\times$3 representing the head rotation in 2D space or 3D space. \\
    5 &  Head translation matrix with the size of 1$\times$2 or 1$\times$3 representing head translation in 2D space or 3D space.  \\
    6 &      Head location matrix with size of 1$\times$2 or 1$\times$3 representing the head location in 2D space or 3D space.  \\
    7 &      Compact feature matrix with the size being specified by gfv_matrix_width_minus1[i] and gfv_matrix_height_minus1[i].  \\
    8$\sim$31 &      Other matrix that may be used as determined by the application with the size being specified by gfv_matrix_width_minus1[i] and gfv_matrix_height_minus1[i].  \\
    32$\sim$64 &      Reserved  \\
  \end{tabular}		
  \\ 
  \Option{SEIGenerativeFaceVideoNumMatricestoNumKpsFlag} &
  \Default{``''} &
  When true, indicates that the number of matrices of the i-th matrix type is equal to  cooridnation number. When false, indicates the number of matrices of the i-th matrix type is not equal to cooridnation number. List of this flag with the corresponding number of specified SEIGenerativeFaceVideoNumMatrixType.
  \\
  \Option{SEIGenerativeFaceVideoNumMatricesInfo} &
  \Default{``''} &
  Provides information to derive the number of the matrices of the i-th matrix type. List of this NumMatricesInfo with the corresponding number of specified SEIGenerativeFaceVideoNumMatrixType.
  \\
  \Option{SEIGenerativeFaceVideoMatrix3DSpaceFlag} &
  \Default{``''} &
  When true, indicates the matrix of the i-th matrix type is a matrix defined in 3D space. When false, indicates the matrix of the i-th matrix type is a matrix defined in 2D space. List of 3D Space Flags with the corresponding number of specified SEIGenerativeFaceVideoNumMatrixType.
  \\
  \Option{SEIGenerativeFaceVideoNumMatrices} &
  \Default{``''} &
  Indicates the number of matrices of the i-th matrix type. List of i-th matrix type with the corresponding number of specified SEIGenerativeFaceVideoNumMatrixType.
  \\
  \Option{SEIGenerativeFaceVideoMatrixWidth} &
  \Default{``''} &
  Indicates  the matrix width of the i-th matrix type. List of i-th matrix width with the corresponding number of specified SEIGenerativeFaceVideoNumMatrixType.
  \\
  \Option{SEIGenerativeFaceVideoMatrixHeight} &
  \Default{``''} &
  Indicates the matrix height of the i-th matrix type. List of i-th matrix height with the corresponding number of specified SEIGenerativeFaceVideoNumMatrixType.
  \\
  \Option{SEIGenerativeFaceVideoMatrixElement} &
  \Default{``''} &
  List of matrix parameters with the floating type.
  \\
  \Option{SEIGenerativeFaceVideoPayloadFilename} &
  \Default{""} &
  Specifies the NNR bitstream of the generative neural network and has the same syntax and semantics with nnpfc_payload_byte of neural network post-filter information URI.
  \\
\end{OptionTableNoShorthand}

\begin{OptionTableNoShorthand}{AI Restrictions Usage SEI message encoder parameters}{tab:sei-aur}
  \Option{SEIAUREnabled} &
  \Default{false} &
  Specifies whether AI restrictions usage SEI is generated or not.
  \\
  \Option{SEIAURCancelFlag} &
  \Default{9} &
  Specifies the value of cancellation flag.
  \\
  \Option{SEIAURPersistenceFlag} &
  \Default{9} &
  Specifies the value of persistence flag.
  \\
  \Option{SEIAURNumRestrictionsMinus1} &
  \Default{9} &
  Specifies the value of number of restrictions minus one.
  \\
  \Option{SEIAURRestrictions} &
  \Default{""} &
  List of restrictions that are applied.
  0 : Do not use in any AI applications
  1 : Do not use for AI training
  2 : Do not use for AI inference
  3 : Do not use for any AI application with generative AI
  4 : Do not use for AI training of generative AI
  5 : Do not use for AI inference with generative AI
  \\
  \Option{SEIAURContextPresentFlag} &
  \Default{""} &
  List of restriction context present flags.
  1st bit : commercial use
  2nd bit : non-commercial use
  3rd bit : official / government use
  4th bit : research and academic use
  \\
  \Option{SEIAURContext} &
  \Default{""} &
  List of restriction context. (Please put space in between values).
  \\
  \Option{SEIAURExclusionFlag} &
  \Default{""} &
  List of flags indicating whether NNPFC, NNPFA, GFV and GFVE SEI messages are excluded from AI usage restrictions.
  \\
\end{OptionTableNoShorthand}

\begin{OptionTableNoShorthand}{Generative face video enhancement SEI message encoder parameters}
{tab:sei-gfve}
  \Option{--} &
  \Default{--} &
  \textbf{The configuration files for generative face video enhancement SEI message can be generated by the AhG16 GFVC common software in https://vcgit.hhi.fraunhofer.de/jvet-ahg-gfvc/gfvc_v1/-/blob/AhG16_GFVC_Software_v3/source/GFVC/codecs_utils.py. }
  \\
  \Option{SEIGenerativeFaceVideoEnhancementEnabled} &
  \Default{false} &
  Enables (true) or disables (false) the insertion of generative face video enhancement (GFVE) information SEI message.
  \\
  \Option{SEIGenerativeFaceVideoEnhancementNumber} &
  \Default{0} &
  Controls how many generative face video enhancement (GFVE) information SEI message to be carried by one base picture.
  \\
  \Option{SEIGenerativeFaceVideoEnhancementBasePicFlag} &
  \Default{false} &
  When true, indicates that the current decoded output picture corresponds to a base picture and this SEI message specifies syntax elements for a base picture. When false, indicates that the current decoded output picture does not correspond to a base picture or this SEI message does not specify syntax elements for a base picture. 
  \\
  \Option{SEIGenerativeFaceVideoEnhancementNNPresentFlag} &
  \Default{false} &
  When true, indicates a neural network that may be used as a EnhancerNN( ) is contained or indicated by the SEI message. When false, indicates a neural network that may be used as a EnhancerNN( ) is not contained or indicated by the SEI message.
  \\
  \Option{SEIGenerativeFaceVideoEnhancementNNModeIdc} &
  \Default{0} &
  Specifies a neural network that may be used as a EnhancerNN() and has the same syntax and semantics with nnpfc_mode_idc of neural network post-filter.
  \\
  \Option{SEIGenerativeFaceVideoEnhancementNNTagURI} &
  \Default{``''} &
  Specifies a neural network that may be used as a EnhancerNN() and has the same syntax and semantics with nnpfc_tag_uri of neural network post-filter identified by a specified tag URI.
  \\
  \Option{SEIGenerativeFaceVideoEnhancementNNURI} &
  \Default{``''} &
  Specifies a neural network that may be used as a EnhancerNN() and has the same syntax and semantics with nnpfc_uri of neural network post-filter information URI.
  \\
  \Option{SEIGenerativeFaceVideoEnhancementId} &
  \Default{`` ''} &
  contains an identifying number that may be used to identify face feature information and specify a neural network that may be used as EnhancerNN( ). The valid range is from \(0\) to \(2^{32} - 2\) (inclusive). Please note that values from \(256\) to \(511\) and from \(2^{31}\) to \(2^{32} - 2\) are reserved for future use, and decoders encountering these values should ignore the corresponding SEI message. Decoders conforming to this edition of this document encountering a GFVE SEI message with gfve\_id in the range of 256 to 511, inclusive, or in the range of \(2^{31}\) to \(2^{32} - 2\) , inclusive, shall ignore the SEI message. List of identifying numbers with the corresponding number of specified SEIGenerativeFaceVideoEnhancementNumber.
  \\
  \Option{SEIGenerativeFaceVideoEnhancementGFVId} &
  \Default{`` ''} &
  specifies gfv\_id and gfv\_cnt of the associated GFV SEI message. The associated GFV SEI message is a GFV SEI message in the same picture unit with the GFVE SEI message having gfv\_id equal to gfve\_gfv\_id, gfv\_cnt equal to gfve\_gfv\_cnt. The GFVE message is used to enhance the picture generated with the associated GFV SEI message. List of identifying numbers with the corresponding number of specified SEIGenerativeFaceVideoEnhancementNumber.
  \\  
  \Option{SEIGenerativeFaceVideoEnhancementGFVCnt} &
  \Default{`` ''} &
  specifies gfv\_id and gfv\_cnt of the associated GFV SEI message. The associated GFV SEI message is a GFV SEI message in the same picture unit with the GFVE SEI message having gfv\_id equal to gfve\_gfv\_id, gfv\_cnt equal to gfve\_gfv\_cnt. The GFVE message is used to enhance the picture generated with the associated GFV SEI message. List of identifying numbers with the corresponding number of specified SEIGenerativeFaceVideoEnhancementNumber.
  \\    
  \Option{SEIGenerativeFaceVideoEnhancementMatrixPresentFlag} &
  \Default{`` ''} &
  When true, indicates that matrix parameters are present. When False, indicates that matrix parameters are not present. List of identifying numbers with the corresponding number of specified SEIGenerativeFaceVideoEnhancementNumber.
  \\   
  \Option{SEIGenerativeFaceVideoEnhancementMatrixPredFlag} &
  \Default{`` ''} &
  Determines whether the enhancement matrix parameter is inter-predicted. List of matrix prediction factors with the corresponding number of specified SEIGenerativeFaceVideoEnhancementNumber.
  \\   
 \Option{SEIGenerativeFaceVideoEnhancement\\MatrixElementPrecisionFactor} &
  \Default{`` ''} &
  Specifies the precision factor (the length \& in bits,) to process the enhancement matrix parameters. The value range may not exceed 32. List of matrix quantization factors with the corresponding number of specified SEIGenerativeFaceVideoEnhancementNumber.
  \\   
  \Option{SEIGenerativeFaceVideoEnhancementNumMatrices} &
  \Default{``''} &
   Specifies the number of matrices signalled in the SEI message. 
  \\
  \Option{SEIGenerativeFaceVideoEnhancementMatrixWidth} &
  \Default{``''} &
  Indicates the matrix width of the i-th matrix type. List of i-th matrix width with the corresponding number of specified SEIGenerativeFaceVideoEnhancementNumMatrices.
  \\
  \Option{SEIGenerativeFaceVideoEnhancementMatrixHeight} &
  \Default{``''} &
  Indicates the matrix height of the i-th matrix type. List of i-th matrix height with the corresponding number of specified SEIGenerativeFaceVideoEnhancementNumMatrices.
  \\
  \Option{SEIGenerativeFaceVideoEnhancementMatrixElement} &
  \Default{``''} &
  List of enhancement matrix parameters with the floating type.
  \\

  \Option{SEIGenerativeFaceVideoEnhancementPupilPresentIdx} &
  \Default{``''} &
  ...
  Equal to 0 indicates the pupil coordinate is not present. Equal to 1 indicates the pupil coordinate information of the left eye is present. Equal to 2 indicates the pupil coordinate information of the right eye is present. Equal to 3 indicates pupil coordinate information of both the left eye and the right eye is present. 
  \\    
  \Option{SEIGenerativeFaceVideoEnhancement \\PupilCoordinatePrecisionFactor} &
  \Default{""} &
  Specifies the precision factor (the length \& in bits,) to process the pupil coordinates. The value range may not exceed 32. List of pupil coordinates quantization factors with the corresponding number of specified SEIGenerativeFaceVideoEnhancementNumber.
  \\
  \Option{SEIGenerativeFaceVideoEnhancement\\PupilLeftEyeCoordinateX} &
  \Default{""} &
  specifies the x-axis coordinate of left eye with the floating type.
  \\
  \Option{SEIGenerativeFaceVideoEnhancement\\PupilLeftEyeCoordinateY} &
  \Default{""} &
  specifies the y-axis coordinate of left eye with the floating type.
  \\
  \Option{SEIGenerativeFaceVideoEnhancement\\PupilRightEyeCoordinateX} &
  \Default{""} &
  specifies the x-axis coordinate of right eye with the floating type.
  \\
  \Option{SEIGenerativeFaceVideoEnhancement\\PupilRightEyeCoordinateY} &
  \Default{""} &
  specifies the y-axis coordinate of right eye with the floating type.
  \\  
\end{OptionTableNoShorthand}
...
\begin{OptionTableNoShorthand}{Packed Regions Info SEI message encoder parameters}{tab:sei-pri}
  ...
  When packed regions info SEI (PRI SEI) is enabled, encoder creates a modified
  picture to be coded that consists of regions extracted from the original
  picture. The regions can be independently scaled and are placed in the picture
  to be coded as instructed by the PRI SEI config parameters. The resulting
  modified picture is encoded and decoded as usual by VTM. Encoder calculates
  the resolution of the coded pictures based on the placement of the regions in
  the coded picture. Encoder or decoder then takes the regions from the decoded
  picture and scales and places regions back at the correct locations in the
  final reconstructed picture. This final reconstruction step is non-normative.
  \\
  ...
  \\
  \Option{SEIPRIMultilayerFlag} &
  \Default{false} &
  Specifies whether layer IDs are signalled.
  \\
 \Option{SEIPRIRegionLayerId} &
  \Default{""} &
  Specifies a list of the layer id of the picture that the region information relate to.
  \\
 \Option{SEIPRIRegionIsALayerFlag} &
  \Default{""} &
  Specifies a list of flags indicating for each region if the picture width and height in the layer are the same as the region's.
  \\
  ...
  \\
  \Option{SEIPRITargetRegionTopLeftInUnitsX} &
  \Default{""} &
  Specifies a list of horizontal top left postions in units for the regions in reconstructed target picture.
  \\
  \Option{SEIPRITargetRegionTopLeftInUnitsY} &
  \Default{""} &
  Specifies a list of vertical top left postions in units for the regions in reconstructed target picture.
  \\
\end{OptionTableNoShorthand}

\begin{OptionTableNoShorthand}{Image Format Metadata SEI message encoder parameters}{tab:sei-ifm}
  \Option{SEIIfmEnabled} &
  \Default{false} &
  Specifies if image format metadata SEI is active.
  \\
  \Option{SEIIfmCancelFlag} &
  \Default{false} &
  Specifies if image format metadata SEI should be cancelled.
  \\
  \Option{SEIIfmPersistenceFlag} &
  \Default{false} &
  Specifies that image format metadata SEI persists beyond the scope of a single picture.
  \\
  \Option{SEIIfmNumMetadataPayloads} &
  \Default{0} &
  Number of image format metadata SEI payloads.
  \\
  \Option{SEIIfmfTypeId\emph{i}} &
  \Default{0} &
  Image format metadata SEI type id of the i-th payload (with i an integer between 0 and 15).
  \\
  \Option{SEIIfmUriPresentFlag\emph{i}} &
  \Default{0} &
  IFM uniform resource identifier present flag of the i-th payload (with i an integer between 0 and 15).
  \\
  \Option{SEIIfmfData\emph{i}} &
  \Default{""} &
  IFM data of the i-th payload (with i an integer between 0 and 15).
  \\
  \Option{SEIIfmURI\emph{i}} &
  \Default{""} &
  URI from which to obtain IFM data when present flag = 1 of the i-th payload (with i an integer between 0 and 15).
  \\
\end{OptionTableNoShorthand}
...
%\ShortOption{\None} &
\Default{\NotSet} &
Packed regions info SEI output file name. If empty, no object information will be saved
\\
